...
def check_miscInfo(config:dict, configDefaults:dict) -> Tuple[dict,dict,bool]:
    ...
    Forcefield = miscInfo.get("forcefield", None)
    if Forcefield is None:
        ## use a default value
        config["miscInfo"]["forcefield"] = configDefaults["miscInfo"]["forcefield"]
        miscInfoDisorders["forcefield"] = "No forcefield specified, using default of a19SB"
    else:
        if not isinstance(Forcefield, dict):
            miscInfoDisorders["forcefield"] = "Forcefield is not formatted correctly in the config file"
            miscInfoOk = False
        else:
...
def check_simulationInfo(config: dict) -> Tuple[dict, bool]:
    ...
    for counter, simulation in enumerate(simulationInfo):
        ...
        elif simulationWithDefaults["simulationType"] in ["ESI"]:
            disorders, mdOptionsOk, simulationWithDefaults  = check_ESI_options(simulationWithDefaults, stepName, disorders)
            allStepsOk *= mdOptionsOk
