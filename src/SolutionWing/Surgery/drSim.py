...
#########################################################################################
def Strip_Solvent(prmtop: app.AmberPrmtopFile, inpcrd: any):
    """
    Remove all solvent atoms
    Parameters
    ----------
    prmtop : file
        The topology file.
    ...
    -------
    prmtop : file
        The changed topology file.
    outcrd : file
        The changed coordinates file.
    """
    # Get the topology and positions from the input files
  
    top = pt.load_topology(prmtop)
    traj= pt.load(inpcrd, top=top)
    stripped_traj = traj.strip(':WAT')
    pt.write_traj(f"stripped_{inpcrd}", stripped_traj, overwrite=True)
    stripped_traj.top.save(f"stripped_{prmtop}")  
