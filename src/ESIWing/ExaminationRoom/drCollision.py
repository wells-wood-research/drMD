...
SHADOWSCREEN_EXE = p.join(THISDIR, "shadowScreenAA")
PROTEIN_SASA_EXE=p.join(THISDIR, "protein_sasa")
PROTEIN_HULL_EXE=p.join(THISDIR, "protein_hull")
print(SHADOWSCREEN_EXE)
if not p.isfile(SHADOWSCREEN_EXE):
    raise PathError ("WHERE IS SHADOWSCREEN???")

print(PROTEIN_SASA_EXE)
if not p.isfile(PROTEIN_SASA_EXE):
    raise PathError ("WHERE IS PROTEIN_SASA???")

print(PROTEIN_HULL_EXE)
if not p.isfile(PROTEIN_HULL_EXE):
    raise PathError ("WHERE IS PROTEIN_HULL???")

def collect_coords(pdbFile, atomMode: str) -> np.ndarray:
    coords = []
    ...
    if not coords:
        raise ValueError(f"No atoms matched the requested mode '{atomMode}'")

    return np.asarray(coords, dtype=float)


def compute_convex_hull(pdbFile, probe_radius: float) -> tuple[float, float, int, int]:
    hullCommand: list = [PROTEIN_HULL_EXE, pdbFile, "hull_output.txt", str(probe_radius)]
    try: 
        ...
        print(errorMessage)
    oputputs= result.stdout.splitlines()
    for line in oputputs:
        if line.startswith("Surface area:"):
            hull_area = (line.split(":")[1].strip()).removesuffix(" Å²")
    #if os.path.exists("hull_output.txt"):
        #os.remove("hull_output.txt")
    return float(hull_area)



def compute_sasa(pdbFile, probe_radius: float) -> float:
    sasaCommand: list = [PROTEIN_SASA_EXE, pdbFile, str(probe_radius)]
    try: 
        # Execute the command and capture its output
        result: subprocess.CompletedProcess[str] = subprocess.run(
            sasaCommand,
    ...
    for line in oputputs:
        if line.startswith("Total SASA:"):
            sasa_values = (line.split(":")[1].strip()).removesuffix(" Å²")

    return float(sasa_values)
...
def visualise_shadow(pdbFile, probe: str) -> float:
    ...
    shadowScreenCommand: list = [SHADOWSCREEN_EXE+"Image", pdbFile, "250", "0.3", "--gas", probe, "--output-dir", outDir, "--debug", "--smooth"]
...
def main(pdbFile, representation, probe, rotations, resolution) -> None:
    ...
    coords = collect_coords(pdbFile, atoms)
    hullTime= time.time()
    hullArea= compute_convex_hull(pdbFile, probe_radius)
    hullTime= time.time() - hullTime

    sasaTime= time.time()
    sasaArea= compute_sasa(pdbFile, probe_radius)
    sasaTime= time.time() - sasaTime

    ccsTime= time.time()
    shadowScreenCommand: list = [SHADOWSCREEN_EXE, pdbFile, str(rotations), str(resolution), "--gas", probe, "--ccs"]
    try: 
        # Execute the command and capture its output
        result: subprocess.CompletedProcess[str] = subprocess.run(
                shadowScreenCommand,
    ...
    ccsCorrected= ccsRaw*(sasaArea/hullArea)
    ccsTime= time.time() - ccsTime
    totalTime= time.time() - totalTime

    get_frame_shadow(pdbFile, representation, probe)
    return ccsCorrected, hullArea, sasaArea
...
def process_ccs_frame_chunk(frameArgs: tuple):
    ...
    for timeStep in frameIndices:
        ...
        ccs, hullArea, sasaArea = main(trajPdb, model, gas, "300", "0.5")
        results.append((timeStep * logTimeStep, ccs, hullArea, sasaArea))
    ...
    for simulationStep in simulationSteps:
        ...
        if p.exists(p.join(ccsDir, "ccsReport.csv")):
            ...
            with open(p.join(ccsDir, "ccsReport.csv"), "w", encoding="utf-8") as f:
                f.write(f"Time ({logTimeUnit}), CCS (Å²), Convex Hull Area (Å²), SASA (Å²)\n")
                for timeValue, ccs, hullArea, sasaArea in sorted(allResults, key=lambda item: item[0]):
                    f.write(f"{timeValue}, {ccs}, {hullArea}, {sasaArea}\n")
