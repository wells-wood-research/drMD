...
def collate_vitals_reports(batchConfig: Dict) -> None:
    """
    Function to collect vitals reports from the trajectories generated by the MD simulations.
    ...
    for systemName in os.listdir(outDir):
        ...
        systemDir = p.join(outDir, systemName)
        if not p.isdir(systemDir):
            continue

        for stepName in os.listdir(systemDir):
            if stepName == "00_prep":
                continue

            vitalsHtml: FilePath = p.join(systemDir, stepName, "vitals_report.html")
            if p.isfile(vitalsHtml):
                copy(vitalsHtml, p.join(vitalsDir, f"{systemName}_{stepName}_vitals.html"))
...
def cluster_handler(batchConfig: Dict) -> None:
    ...
    drMethodsWriter.add_analysis_step_to_log("clustering")
    drMethodsWriter.add_parameter_to_analysis_log("clustering", "stepNames", clusterInfo.get("stepNames"))
    drMethodsWriter.add_parameter_to_analysis_log("clustering", "clusterMethod", "kmeans")
    drMethodsWriter.add_parameter_to_analysis_log("clustering", "removeAtoms", removeAtoms)
    drMethodsWriter.add_parameter_to_analysis_log("clustering", "clusterBy", clusterInfo.get("clusterBy"))
    drMethodsWriter.add_parameter_to_analysis_log("clustering", "nClusters", clusterInfo.get("nClusters"))
...
def remove_step_directories(batchConfig: dict, stepsToRemove: list) -> None:
    ...
    dirsToRemove: List[DirectoryPath] = [
        p.join(outDir, p.splitext(file)[0], stepName)
...
def get_endpoint_pdbs(endPointInfo: Dict, pathInfo: Dict) -> List[FilePath]:
    ...
    runDirs: List[DirectoryPath] = [p.join(outDir, dir) for dir in os.listdir(outDir) if dir not in notRunDirs]
    ## create a list of step directories that we want to look for endpoint pdb files in
    stepDirs: List[DirectoryPath] = [
        p.join(runDir, stepDir)
        for runDir in runDirs
        for stepDir in endPointInfo["stepNames"]
        if p.isdir(p.join(runDir, stepDir))
    ]
    ## create a list of endpoint pdb files
    endpointPdbs: List[FilePath] = [
        p.join(stepDir, stepPdb)
