...
##################################################################################################
def _legacy_reset_atom_numbers(pdbFile: str) -> str:
    """
...
def _legacy_reset_chains(refPdb: FilePath, inputPdb: FilePath, ligandNames = []) -> FilePath:
...
def _legacy_reset_chains_residues(templatePdb: FilePath, inputPdb: FilePath, config: dict) -> FilePath:
    """
    ...
    return inputPdb
##################################################################################################
def _legacy_reset_water_ions(inputDf: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    chainIds = nonWaterIonsDf["CHAIN_ID"].dropna().astype(str).str.strip()
    chainIds = chainIds[chainIds.str.len() == 1]
    maxChain = max(chainIds, default="")

    if maxChain in ["Y", "Z"] or not maxChain:
        ionChain = " "
        waterChain = " "
    else:
    ...
    return outputDf
##################################################################################################
def _legacy_reset_chain_residues_ligands(templateDf: pd.DataFrame, inputDf: pd.DataFrame, ligandNames: list) -> pd.DataFrame:
    """
    ...
    for (inputChain, inputChainDf), (templateChain, templateChainDf) in zip(inputLigandsDf.groupby("CHAIN_ID"), templateLigandsDf.groupby("CHAIN_ID")):
        for (inputRes, inputResDf), (templateRes, templateResDf) in zip(inputChainDf.groupby("RES_ID"), templateChainDf.groupby("RES_ID")):
            ...
            inputResidueMask = (
                (inputDf["CHAIN_ID"] == inputChain) &
                (inputDf["RES_ID"] == inputRes) &
                (inputDf["RES_NAME"].isin(inputResDf["RES_NAME"].unique()))
            )
            outputDf.loc[inputResidueMask, "CHAIN_ID"] = templateChain
            outputDf.loc[inputResidueMask, "RES_ID"] = templateRes

    return outputDf

def _legacy_account_for_no_CA_ncaas(pdbDf: pd.DataFrame, ncaaNames: list[str]) -> pd.DataFrame:
...
##################################################################################################
def _legacy_reset_chain_residues_protein(templateDf: pd.DataFrame, inputDf: pd.DataFrame, ncaaNames: list[str]) -> pd.DataFrame:
    """
    ...
    for templateCa, inputCa in zip(templateCaDf.iterrows(), inputCaDf.iterrows()):
        ...
        inputResidueMask = (
            (inputDf["CHAIN_ID"] == inputChainId) &
            (inputDf["RES_ID"] == inputResidueId) &
            (inputDf["RES_NAME"] == inputResidueName)
        )
        outputDf.loc[inputResidueMask, "CHAIN_ID"] = targetChainId
        outputDf.loc[inputResidueMask, "RES_ID"] = targetResidueId
    return outputDf

# ##################################################################################################
def _legacy_fix_atom_names(df): 
...
def _legacy_parse_pdb_atom_line(line: str) -> dict | None:
    ...
    try:
        atom_serial = _decode_pdb_integer(line[6:11], default=1)
        atom_name = line[12:16].strip()
        res_name = line[17:20].strip()
        chain_id = line[21:22].strip() or "A"
        res_id = _decode_pdb_integer(line[22:26], default=1)
    ...
    try:
        atom_serial = _decode_pdb_integer(tokens[1], default=1)
        atom_name = tokens[2]
        res_name = tokens[3]
...
def _legacy_sanitize_pdb_lines(pdb_text: str) -> str:
...
def _legacy_fit_pdb_field(value: object, width: int, *, align: str = "right") -> str:
...
def _legacy_normalize_residue_id(res_id: object) -> int:
...
def _format_pdb_coordinate(value: object) -> str:
    """Format a coordinate so it stays inside the 8-character PDB field."""
    try:
        number = float(value)
    except (TypeError, ValueError):
        number = 0.0

    if number > 999.999 or number < -99.999:
        field = f"{number:8.2f}"
    else:
        field = f"{number:8.3f}"
...
def _decode_pdb_integer(value: object, *, default: int = 1) -> int:
    """Decode a decimal or hybrid-36 PDB integer field."""
    ...
    for character in text.upper():
        digit = int(character) if character.isdigit() else ord(character) - ord("A") + 10
        value_base36 = value_base36 * 36 + digit
    width = len(text)
    return sign * (10 ** width + value_base36 - 10 * (36 ** (width - 1)))


def _encode_pdb_integer(value: object, width: int) -> str:
    """Encode a PDB integer as decimal or hybrid-36 when decimal overflows."""
    number = int(value)
    decimal_limit = 10 ** width
    if 0 <= number < decimal_limit:
        return f"{number:>{width}d}"
    if number < 0:
        raise ValueError(f"Negative PDB counter is not supported: {number}")

    hybrid = number - decimal_limit + 10 * (36 ** (width - 1))
    digits = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    encoded = ""
    for _ in range(width):
        hybrid, remainder = divmod(hybrid, 36)
        encoded = digits[remainder] + encoded
    if hybrid:
        raise ValueError(f"PDB counter exceeds hybrid-36 capacity: {number}")
    return encoded
...
def format_pdb_atom_record(
    *,
    ...
    line[6:11] = list(_encode_pdb_integer(atom_serial, 5))
    ...
    line[22:26] = list(_encode_pdb_integer(res_id, 4))
    ...
    # 31-38: x
    line[30:38] = list(_format_pdb_coordinate(x))

    # 39-46: y
    line[38:46] = list(_format_pdb_coordinate(y))

    # 47-54: z
    line[46:54] = list(_format_pdb_coordinate(z))
...
def _repair_malformed_atom_line(line: str) -> str:
    ...
    try:
        ...
        return write_pdb_atom(
            ...
            res_name=res_name,
            chain=chain_id,
            res_num=_decode_pdb_integer(res_id),
            x=x,
            y=y,
            z=z,
            bfactor=bfactor,
