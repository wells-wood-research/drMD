...
def _repair_malformed_atom_line(line: str) -> str:
    ...
    try:
        ...
        if len(tokens) >= 11 and tokens[4].isalpha() and len(tokens[4]) == 1:
            ...
            bfactor = float(tokens[10]) if len(tokens) > 10 else 0.0
            element = tokens[11] if len(tokens) > 11 else atom_name[0].upper()
        else:
            chain_id = "A"
            ...
            bfactor = float(tokens[9]) if len(tokens) > 9 else 0.0
        ...
        return write_pdb_atom(
            ...
            res_name=res_name,
            chain=chain_id,
            res_num=int(res_id) if str(res_id).strip().isdigit() else 1,
            x=x,
            y=y,
            z=z,
            bfactor=bfactor,
            ...
            if parsed is None:
                ...
                fixed_lines.append(
                    write_pdb_atom(
                        ...
                        res_num=int(str(parsed["res_id"]).strip() or 1),
                        x=parsed["x"],
                        y=parsed["y"],
                        z=parsed["z"],
...
def renumber_pdb_atom_serials(pdb_path: str) -> None:
    """Reset both atom serials and residue numbers to a clean 1-based ordering."""
    repair_pdb_file(pdb_path)

    with open(pdb_path, "r", encoding="utf-8", errors="ignore") as fh:
        original = fh.read()

    lines = sanitize_pdb_lines(original).splitlines()
    out_lines = []
    next_serial = 1
