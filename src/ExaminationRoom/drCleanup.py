...
def collate_vitals_reports(batchConfig: Dict) -> None:
    ...
    for systemName in os.listdir(outDir):
        ...
        for stepName in os.listdir(p.join(outDir, systemName)):
            if stepName == "00_prep":
                continue
            vitalsPdf: FilePath = p.join(outDir, systemName, stepName, "vitals_report.pdf")
            if p.isfile(vitalsPdf):
                vitalsPdfDestination: FilePath = p.join(vitalsDir, f"{systemName}_{stepName}_vitals.pdf")
                copy(vitalsPdf, vitalsPdfDestination)
